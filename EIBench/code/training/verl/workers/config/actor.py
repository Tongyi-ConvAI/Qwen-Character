...
class ActorConfig(BaseConfig):
    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None, skip_batch_size_check: bool = False):
        """Validate actor configuration with runtime parameters."""
        if not self.use_dynamic_bsz:
            if train_batch_size < self.ppo_mini_batch_size:
                if skip_batch_size_check:
                    import warnings
                    warnings.warn(
                        f"train_batch_size ({train_batch_size}) < actor.ppo_mini_batch_size "
                        f"({self.ppo_mini_batch_size}), allowed because strip-think expansion "
                        f"will produce enough samples at runtime."
                    )
                else:
                    raise ValueError(
...
class FSDPActorConfig(ActorConfig):
    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None, skip_batch_size_check: bool = False):
        ...
        super().validate(n_gpus, train_batch_size, model_config, skip_batch_size_check=skip_batch_size_check)
