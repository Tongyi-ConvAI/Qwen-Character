...
class MegatronPPOActor(BasePPOActor):
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        if self.config.shuffle:
            assert self.config.data_loader_seed is not None, "If shuffle dataloader, seed must be manually set"

        print(
            f"[megatron_actor] INFO: using ragged minibatch split for PPO update. "
        ...
        def get_data():
            for epoch in range(self.config.ppo_epochs):
                ...
                if self.config.shuffle:
                    ...
                    generator.manual_seed(int(self.config.data_loader_seed) + epoch)
                    indices = torch.randperm(batch_size, generator=generator)
                    epoch_data = data[indices]

                for start in range(0, len(epoch_data), mini_batch_size):
                    yield epoch_data[start : start + mini_batch_size]
