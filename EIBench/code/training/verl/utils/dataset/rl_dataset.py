...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.stratified_batch = config.get("stratified_batch", False)
        self.stratified_key = config.get("stratified_key", "data_source")
    ...
    def _read_files_and_tokenize(self):
        ...
        # ── Stratified interleaving: round-robin by data_source ──
        if self.stratified_batch and self.stratified_key in self.dataframe.column_names:
            import random as _rnd
            rng = _rnd.Random(self.seed if self.seed is not None else 42)
            col = self.dataframe[self.stratified_key]
            groups: dict[str, list[int]] = {}
            for idx, src in enumerate(col):
                groups.setdefault(src, []).append(idx)
            for indices in groups.values():
                rng.shuffle(indices)
            group_iters = [iter(indices) for indices in groups.values()]
            interleaved: list[int] = []
            while group_iters:
                exhausted = []
                for i, it in enumerate(group_iters):
                    try:
                ...
                for i in reversed(exhausted):
                    group_iters.pop(i)
            self.dataframe = self.dataframe.select(interleaved)
            src_names = sorted(groups.keys())
            print(f"Stratified interleave by '{self.stratified_key}': {len(src_names)} sources "
                  f"({', '.join(f'{s}={len(groups[s])}' for s in src_names)}), "
                  f"total={len(interleaved)}")
