...
def _flatten_metric_values(value: Any) -> list[Any]:
    if isinstance(value, Metric):
        return _flatten_metric_values(value.aggregate())
    if isinstance(value, torch.Tensor):
        if value.numel() == 1:
            return [value.detach().item()]
        return _flatten_metric_values(value.detach().cpu().tolist())
    if isinstance(value, np.ndarray):
        if value.size == 1:
            return [value.item()]
        return _flatten_metric_values(value.tolist())
...
def reduce_metrics(metrics: dict[str, Union["Metric", list[Any]]]) -> dict[str, Any]:
    ...
    for key, val in metrics.items():
        ...
        if len(flattened) == 0:
            raise ValueError(f"Metric '{key}' is empty and cannot be reduced")
        try:
            arr = np.asarray(flattened, dtype=np.float64)
        except Exception as exc:
            raise ValueError(
                f"Metric '{key}' contains non-scalar values after flattening: {type(val)} -> {flattened[:3]}"
            ) from exc
        if "max" in key:
            metrics[key] = np.max(arr)
        elif "min" in key:
            metrics[key] = np.min(arr)
        else:
            metrics[key] = np.mean(arr)
    return metrics
