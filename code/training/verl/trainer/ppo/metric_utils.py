...
REWARD_EXTRA_NUMERIC_KEYS = (
    "empty_think_ratio",
    "think_present_ratio",
    "missing_think_turns",
    "malformed_think_turns",
    "orphan_think_open_tags",
    "orphan_think_close_tags",
    "empty_response_ratio",
    "empty_response_turns",
    "parse_bad_turn_ratio",
    "parse_bad_turns",
    "format_bad_turn_ratio",
    "format_bad_turns",
    "hard_format_veto",
    "empty_think_penalty",
    "parse_error_penalty_applied",
    "format_error_penalty_applied",
    "unfinished_streak_final",
    "template_repeat_streak_final",
    "total_turn_count",
    "all_turns_have_think",
    "any_turn_missing_think",
)

REWARD_EXTRA_WEIGHTED_RATIO_SPECS = {
    "format/empty_visible_rate_weighted": ("empty_response_turns", "total_turn_count"),
    "format/missing_think_rate_weighted": ("missing_think_turns", "total_turn_count"),
    "format/malformed_think_rate_weighted": ("malformed_think_turns", "total_turn_count"),
    "format/parse_bad_turn_rate_weighted": ("parse_bad_turns", "total_turn_count"),
    "format/format_bad_turn_rate_weighted": ("format_bad_turns", "total_turn_count"),
}

REWARD_EXTRA_ALIAS_KEYS = {
    "hard_format_veto": "format/hard_format_veto_rate",
    "empty_response_ratio": "format/empty_visible_rate",
    "empty_think_ratio": "format/empty_think_rate",
    "parse_bad_turn_ratio": "format/parse_bad_turn_rate",
    "format_bad_turn_ratio": "format/format_bad_turn_rate",
}
...
def _safe_weighted_ratio(numerator: np.ndarray, denominator: np.ndarray) -> float | None:
    n = min(numerator.size, denominator.size)
    if n <= 0:
        return None
    ...
    total_denominator = float(np.sum(denominator[valid]))
    if total_denominator <= 0:
        return None
    return float(np.sum(numerator[valid]) / total_denominator)


def _compute_per_source_mean(values_np: np.ndarray, data_sources: list[Any]) -> dict[str, float]:
    n = min(values_np.size, len(data_sources))
    if n <= 0:
        return {}

    per_source_metric: dict[str, list[float]] = defaultdict(list)
    for ds, value in zip(data_sources[:n], values_np[:n], strict=True):
        if np.isnan(value):
            continue
        per_source_metric[str(ds)].append(float(value))

    return {
        str(ds): float(np.mean(source_values))
        for ds, source_values in per_source_metric.items()
        if len(source_values) > 0
    }


def _compute_per_source_weighted_ratio(
    numerator: np.ndarray,
    denominator: np.ndarray,
    data_sources: list[Any],
) -> dict[str, float]:
    n = min(numerator.size, denominator.size, len(data_sources))
    if n <= 0:
        return {}

    per_source_num: dict[str, float] = defaultdict(float)
    per_source_den: dict[str, float] = defaultdict(float)
    for ds, num, den in zip(data_sources[:n], numerator[:n], denominator[:n], strict=True):
        if np.isnan(num) or np.isnan(den) or den <= 0:
            continue
        key = str(ds)
        per_source_num[key] += float(num)
        per_source_den[key] += float(den)

    return {
        ds: per_source_num[ds] / per_source_den[ds]
        for ds in per_source_num
        if per_source_den[ds] > 0
    }
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Legacy simulator/url-environment goal metrics compatibility.
    if batch.non_tensor_batch:
        goal_scores = batch.non_tensor_batch.get("goal_score")
        if goal_scores is not None:
            goal_scores_np = _to_float_array(goal_scores)
            valid = ~np.isnan(goal_scores_np)
            if np.any(valid):
                metrics["goal/score_mean"] = float(np.mean(goal_scores_np[valid]))

        dialogue_turns = batch.non_tensor_batch.get("dialogue_turns")
        if dialogue_turns is None:
            dialogue_turns = batch.non_tensor_batch.get("__num_turns__")
        if dialogue_turns is not None:
            dialogue_turns_np = _to_float_array(dialogue_turns)
            valid = ~np.isnan(dialogue_turns_np)
            if np.any(valid):
                metrics["goal/avg_turns"] = float(np.mean(dialogue_turns_np[valid]))

        anger = batch.non_tensor_batch.get("anger")
        trust = batch.non_tensor_batch.get("trust")
        target_anger = batch.non_tensor_batch.get("target_anger")
        target_trust = batch.non_tensor_batch.get("target_trust")
        emotion_reward = batch.non_tensor_batch.get("emotion_reward")
        if emotion_reward is None:
            emotion_reward = batch.non_tensor_batch.get("trust_reward")
        if anger is not None and target_anger is not None:
            anger_np = _to_float_array(anger)
            target_anger_np = _to_float_array(target_anger)
            n = min(anger_np.size, target_anger_np.size)
            if n > 0:
                valid = ~np.isnan(anger_np[:n]) & ~np.isnan(target_anger_np[:n])
                if np.any(valid):
                    metrics["goal/anger_hit_ratio"] = float(np.mean(anger_np[:n][valid] <= target_anger_np[:n][valid]))

        if trust is not None and target_trust is not None:
            trust_np = _to_float_array(trust)
            target_trust_np = _to_float_array(target_trust)
            n = min(trust_np.size, target_trust_np.size)
            if n > 0:
                valid = ~np.isnan(trust_np[:n]) & ~np.isnan(target_trust_np[:n])
                if np.any(valid):
                    metrics["goal/trust_hit_ratio"] = float(np.mean(trust_np[:n][valid] >= target_trust_np[:n][valid]))

        if anger is not None and trust is not None and target_anger is not None and target_trust is not None:
            anger_np = _to_float_array(anger)
            trust_np = _to_float_array(trust)
            target_anger_np = _to_float_array(target_anger)
            target_trust_np = _to_float_array(target_trust)
            n = min(anger_np.size, trust_np.size, target_anger_np.size, target_trust_np.size)
            if n > 0:
                valid = (
                    ~np.isnan(anger_np[:n])
                    & ~np.isnan(trust_np[:n])
                    & ~np.isnan(target_anger_np[:n])
                    & ~np.isnan(target_trust_np[:n])
                )
                if np.any(valid):
                    success = (anger_np[:n][valid] <= target_anger_np[:n][valid]) & (
                        trust_np[:n][valid] >= target_trust_np[:n][valid]
                    )
                    metrics["goal/success_ratio"] = float(np.mean(success))

        data_sources = batch.non_tensor_batch.get("data_source")
        if data_sources is None:
            data_sources = batch.non_tensor_batch.get("scene_tag")
        ...
        if data_sources is not None and not isinstance(data_sources, list):
            data_sources = [data_sources] * int(sequence_score.shape[0])

        reward_extra_arrays = {
            key: _to_float_array(batch.non_tensor_batch.get(key))
            for key in REWARD_EXTRA_NUMERIC_KEYS
            if batch.non_tensor_batch.get(key) is not None
        }

        for key, values_np in reward_extra_arrays.items():
            valid = ~np.isnan(values_np)
            if np.any(valid):
                mean_value = float(np.mean(values_np[valid]))
                metrics[f"reward_extra/{key}"] = mean_value
                alias_key = REWARD_EXTRA_ALIAS_KEYS.get(key)
                if alias_key is not None:
                    metrics[alias_key] = mean_value

        if data_sources is not None:
            rewards_np = sequence_score.detach().cpu().numpy()
            n = min(len(data_sources), len(rewards_np))
            if n > 0:
                per_source_reward: dict[str, list[float]] = defaultdict(list)
                for ds, reward in zip(data_sources[:n], rewards_np[:n], strict=True):
                    per_source_reward[str(ds)].append(float(reward))
                for ds, rewards in per_source_reward.items():
                    metrics[f"outcome_reward/{ds}"] = float(np.mean(rewards))

                if emotion_reward is not None:
                    emotion_reward_np = _to_float_array(emotion_reward)
                    n_emotion = min(len(data_sources), emotion_reward_np.size)
                    if n_emotion > 0:
                        valid_emotion = ~np.isnan(emotion_reward_np[:n_emotion])
                        if np.any(valid_emotion):
                            metrics["emotion_reward"] = float(np.mean(emotion_reward_np[:n_emotion][valid_emotion]))

                        per_source_emotion: dict[str, list[float]] = defaultdict(list)
                        for ds, score in zip(data_sources[:n_emotion], emotion_reward_np[:n_emotion], strict=True):
                            if np.isnan(score):
                                continue
                            per_source_emotion[str(ds)].append(float(score))
                        for ds, scores in per_source_emotion.items():
                            metrics[f"emotion_reward/{ds}"] = float(np.mean(scores))

                # Per-scene anger/trust/success breakdown
                anger_arr = _to_float_array(anger) if anger is not None else None
                trust_arr = _to_float_array(trust) if trust is not None else None
                target_anger_arr = _to_float_array(target_anger) if target_anger is not None else None
                target_trust_arr = _to_float_array(target_trust) if target_trust is not None else None

                if anger_arr is not None and trust_arr is not None:
                    per_src_anger: dict[str, list[float]] = defaultdict(list)
                    per_src_trust: dict[str, list[float]] = defaultdict(list)
                    for idx_i in range(min(n, anger_arr.size, trust_arr.size)):
                        ds_i = str(data_sources[idx_i])
                        a_i, t_i = anger_arr[idx_i], trust_arr[idx_i]
                        if not np.isnan(a_i):
                            per_src_anger[ds_i].append(float(a_i))
                        if not np.isnan(t_i):
                            per_src_trust[ds_i].append(float(t_i))
                    for ds_i, vals in per_src_anger.items():
                        metrics[f"anger_mean/{ds_i}"] = float(np.mean(vals))
                    for ds_i, vals in per_src_trust.items():
                        metrics[f"trust_mean/{ds_i}"] = float(np.mean(vals))

                if (
                    anger_arr is not None
                    and trust_arr is not None
                    and target_anger_arr is not None
                    and target_trust_arr is not None
                ):
                    per_src_success: dict[str, list[float]] = defaultdict(list)
                    nn = min(n, anger_arr.size, trust_arr.size, target_anger_arr.size, target_trust_arr.size)
                    for idx_i in range(nn):
                        ds_i = str(data_sources[idx_i])
                        a_i = anger_arr[idx_i]
                        t_i = trust_arr[idx_i]
                        ta_i = target_anger_arr[idx_i]
                        tt_i = target_trust_arr[idx_i]
                        if np.isnan(a_i) or np.isnan(t_i) or np.isnan(ta_i) or np.isnan(tt_i):
                            continue
                        per_src_success[ds_i].append(float((a_i <= ta_i) and (t_i >= tt_i)))
                    for ds_i, vals in per_src_success.items():
                        metrics[f"success_ratio/{ds_i}"] = float(np.mean(vals))

                if goal_scores is not None:
                    goal_scores_np = _to_float_array(goal_scores)
                    n_goal = min(n, goal_scores_np.size)
                    if n_goal > 0:
                        per_source_goal: dict[str, list[float]] = defaultdict(list)
                        for ds, score in zip(data_sources[:n_goal], goal_scores_np[:n_goal], strict=True):
                            if np.isnan(score):
                                continue
                            per_source_goal[str(ds)].append(float(score))
                        for ds, scores in per_source_goal.items():
                            metrics[f"goal/score_mean/{ds}"] = float(np.mean(scores))

            for key, values_np in reward_extra_arrays.items():
                per_source_means = _compute_per_source_mean(values_np=values_np, data_sources=data_sources)
                for ds, mean_value in per_source_means.items():
                    metrics[f"reward_extra/{key}/{ds}"] = mean_value
                    alias_key = REWARD_EXTRA_ALIAS_KEYS.get(key)
                    if alias_key is not None:
                        metrics[f"{alias_key}/{ds}"] = mean_value

            malformed_turns_np = reward_extra_arrays.get("malformed_think_turns")
            total_turn_count_np = reward_extra_arrays.get("total_turn_count")
            if malformed_turns_np is not None and total_turn_count_np is not None:
                malformed_rate = _safe_weighted_ratio(malformed_turns_np, total_turn_count_np)
                if malformed_rate is not None:
                    metrics["format/malformed_think_rate"] = malformed_rate
                per_source_malformed = _compute_per_source_weighted_ratio(
                    numerator=malformed_turns_np,
                    denominator=total_turn_count_np,
                    data_sources=data_sources,
                )
                for ds, ratio in per_source_malformed.items():
                    metrics[f"format/malformed_think_rate/{ds}"] = ratio

            for metric_name, (numerator_key, denominator_key) in REWARD_EXTRA_WEIGHTED_RATIO_SPECS.items():
                numerator = reward_extra_arrays.get(numerator_key)
                denominator = reward_extra_arrays.get(denominator_key)
                if numerator is None or denominator is None:
                    continue

                weighted_ratio = _safe_weighted_ratio(numerator, denominator)
                if weighted_ratio is not None:
                    metrics[metric_name] = weighted_ratio

                per_source_ratio = _compute_per_source_weighted_ratio(
                    numerator=numerator,
                    denominator=denominator,
                    data_sources=data_sources,
                )
                for ds, ratio in per_source_ratio.items():
                    metrics[f"{metric_name}/{ds}"] = ratio
