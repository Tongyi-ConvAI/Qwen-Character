...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        # std_min: floor on group std to prevent advantage explosion
        # when all rollouts in a group get similar rewards (std→0)
        std_min = float(config.get("grpo_std_min", 0.0)) if config is not None else 0.0
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                clamped_std = torch.clamp(id2std[index[i]], min=std_min) if std_min > 0 else id2std[index[i]]
                scores[i] = (scores[i] - id2mean[index[i]]) / (clamped_std + epsilon)
            else:
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        std_min = float(config.get("grpo_std_min", 0.0)) if config is not None else 0.0
        if norm_adv_by_std_in_grpo:
            clamped_std_g = torch.clamp(std_g, min=std_min) if std_min > 0 else std_g
            scalars = (scores - mean_g[g]) / (clamped_std_g[g] + epsilon)
        else:
...
@register_adv_est(AdvantageEstimator.RAFT)
def compute_raft_outcome_advantage(
    ...
    **kwargs,
    ...
    """RAFT (Reward-ranked Fine-Tuning): use reward directly as advantage, zero out negatives."""
    with torch.no_grad():
        scores = token_level_rewards.sum(dim=-1)
        scores[scores < 0] = 0.0
