#!/usr/bin/env bash
# =============================================================================
# CTC-GRPO training: Qwen3-8B on EIBench (single node, 8 GPUs, FSDP + vLLM).
#
# Reproduces the 8B run in the paper: GRPO with sigma_min=0.1 + entropy=0.001 +
# centered turn-credit (alpha=15), Qwen3-Max as the user simulator.
#
# Fill in the 3 starred vars, then:  bash scripts/train_8b.sh
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TRAIN_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"          # .../code/training
CODE_DIR="$(cd "$TRAIN_DIR/.." && pwd)"
REPO_ROOT="$(cd "$CODE_DIR/.." && pwd)"
SRC_DIR="${CODE_DIR}/src"                          # holds simulator_v2.py

# ---- python env (deps from requirements.txt; verl is the in-repo fork) ----
RL_VENV="${RL_VENV:-}"                             # optional: path to a venv activate script
[ -n "$RL_VENV" ] && [ -f "$RL_VENV" ] && source "$RL_VENV"
export PYTHONPATH="${TRAIN_DIR}:${SRC_DIR}:${PYTHONPATH:-}"   # cleaned verl + simulator_v2 first

# ============================================================================
# ★★★ FILL THESE IN ★★★
# ============================================================================
MODEL_PATH="${MODEL_PATH:-/path/to/Qwen3-8B}"                          # ★ local HF model dir
...
# ============================================================================
# Hyperparameters (paper 8B run)
# ============================================================================
ACTOR_LR="1e-6"; LR_WARMUP_RATIO="0.03"; MIN_LR_RATIO="0.1"; CLIP_RATIO="0.2"; ENTROPY_COEFF="0.001"
PROMPT_LEN="8192"; RESPONSE_LEN="1536"
TRAIN_BATCH_SIZE="16"; VAL_BATCH_SIZE="32"; ROLLOUT_N="8"
PPO_MINI_BATCH_SIZE="32"; PPO_MICRO_BATCH_SIZE_PER_GPU="4"
ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU="12288"; ROLLOUT_TP="1"
TURN_CREDIT_ALPHA="15.0"; GRPO_STD_MIN="0.1"          # CTC-GRPO core
TOTAL_STEPS="${TOTAL_STEPS:-250}"; MAX_TURNS="8"
N_GPUS_PER_NODE="8"

# ============================================================================
# Data (EIBench train split for training, test split for validation)
# ============================================================================
...
OUT_DIR="${OUT_DIR:-$TRAIN_DIR/ckpt_8b}"
mkdir -p "$OUT_DIR"
...
export WANDB_MODE="${WANDB_MODE:-offline}"     # set online + WANDB_API_KEY to log to W&B
...
# ---- start single-node Ray ----
ray stop --force 2>/dev/null || true
ray start --head --port=6379 --disable-usage-stats --num-gpus=${N_GPUS_PER_NODE} --temp-dir=/tmp/ray_eibench

echo "=== CTC-GRPO 8B: steps=${TOTAL_STEPS}, alpha=${TURN_CREDIT_ALPHA}, sigma_min=${GRPO_STD_MIN}, simulator=${SIMULATOR_MODEL} ==="
...
python3 -m verl.trainer.main_ppo \
  ...
  trainer.experiment_name="ctc_grpo_8b" \
